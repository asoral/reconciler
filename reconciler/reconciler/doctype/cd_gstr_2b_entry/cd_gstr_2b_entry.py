...
def unlink_pr(doc_name):
	...
	doc.cf_match_status = 'Missing in PI'
	doc.cf_reason = None
...
def rematch_result(doc_name):
	...
	if res:
		if doc.cf_match_status:
			if not doc.cf_match_status == res[1]:
				if match_status_priority_list.index(doc.cf_match_status) < \
					...
					frappe.msgprint(_("Existing PI is the best match for this record"))
				else:
		...
		if doc.cf_match_status == 'Missing in PI':
			frappe.throw(_("No PI matched for the given 2B record"))
